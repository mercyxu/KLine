...
- (void)setHttpHeader {
    ...
    [self.requestSerializer setValue:@"bhexApp" forHTTPHeaderField:@"app"];
    [self.requestSerializer setValue:KUser.netWorkStatus forHTTPHeaderField:@"nett"];
    [self.requestSerializer setValue:[self userAgent] forHTTPHeaderField:@"User-Agent"];
    [self.requestSerializer setValue:[infoDic objectForKey:@"CFBundleShortVersionString"] forHTTPHeaderField:@"appVersion"];
    if (KUser.userId && KUser.isLogin) {
        [self.requestSerializer setValue:KUser.userId forHTTPHeaderField:@"userId"];
    }
    [self.requestSerializer setValue:[NSString stringWithFormat:@"iOS %@",device.systemVersion] forHTTPHeaderField:@"osVersion"];
    ...
    [self.requestSerializer setValue:[FCUUID uuidForDevice] forHTTPHeaderField:@"imei"];
    [self.requestSerializer setValue:[[LocalizeHelper sharedLocalSystem] getRequestHeaderLanguageCode] forHTTPHeaderField:@"Accept-Language"];
    [self.requestSerializer setValue:KConfigure.channel forHTTPHeaderField:@"channel"];
    [self.requestSerializer setValue:KConfigure.channel forHTTPHeaderField:@"source"];
}
    ...
    switch (method) {
        case Get:{
            [self GET:aPath parameters:[self getPostRequestSignatureDictionary:params] progress:^(NSProgress * _Nonnull downloadProgress) {
                ...
                [BHDomainHelper sharedDomainHelper].errorCount = 0;
                ...
                if (statusCode == 400) {
                    ...
                    [[BHDomainHelper sharedDomainHelper] changeDomain];
                    NSLog(@"证书无效！");
                } else {
