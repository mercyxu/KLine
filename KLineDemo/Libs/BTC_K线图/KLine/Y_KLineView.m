...
/** 横线 */
@property (strong, nonatomic) UIView *transverseLine;
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress
{
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state)
    {
        ...
        if (!self.transverseLine) {
            self.transverseLine = [UIView new];
            self.transverseLine.clipsToBounds = YES;
            [self.scrollView addSubview:self.transverseLine];
            self.transverseLine.backgroundColor = [UIColor longPressLineColor];
            self.transverseLine.frame = CGRectMake(0, 0, kScreen_Width - 50, Y_StockChartLongPressVerticalViewWidth);
        }
        
        //更新竖线位置
        CGPoint point = [self.kLineMainView getExactXPositionWithOriginXPosition:location.x];
        [self.verticalView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(@(point.x));
        }];
        
        self.transverseLine.left = self.scrollView.contentOffset.x;
        self.transverseLine.centerY = point.y + kLineMainTop;
