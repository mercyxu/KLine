...
- (NSNumber *)MA5 {
    if([Y_StockChartGlobalVariable mainViewLineStatus] == Y_StockChartTargetLineStatusMA) {
        if (!_MA10) {
            ...
            if (index >= 4) {
                if (index > 4) {
                    _MA5 = @((self.SumOfLastClose.doubleValue - self.ParentGroupModel.models[index - 5].SumOfLastClose.doubleValue) / 5);
                } else {
...
{
    if(!_Volume_EMA5) {
        _Volume_EMA5 = @((self.Volume * 2 + 4 * self.PreviousKlineModel.Volume_EMA5.doubleValue)/6);
    }
    return _Volume_EMA5;
}
...
- (NSNumber *)BOLL_MD {
    ...
    if (!_BOLL_MD) {
        ...
        if (index >= 20) {
            ...
            double mdtemp = (self.PreviousKlineModel.BOLL_SUBMD_SUM.doubleValue - self.ParentGroupModel.models[index - 20].BOLL_SUBMD_SUM.doubleValue)/ 20;
            _BOLL_MD = @(sqrt(mdtemp));
...
- (NSNumber *)BOLL_SUBMD{
    ...
    // NSLog(@"lazy_BOLL_SUBMD: \n MA20: %@ \n Close: %@ \n subNum: %f", _MA20, _Close, self.Close.doubleValue - self.MA20.doubleValue);
...
- (void) initWithDataDict:(NSDictionary *)dict
{
    
    if (self)
    {
        _Date = dict[@"t"];
        _Open = @([dict[@"o"] doubleValue]);
        _High = @([dict[@"h"] doubleValue]);
        _Low = @([dict[@"l"] doubleValue]);
        _Close = @([dict[@"c"] doubleValue]);
        _Volume = [dict[@"v"] doubleValue];
        self.SumOfLastClose = @(_Close.doubleValue + self.PreviousKlineModel.SumOfLastClose.doubleValue);
        self.SumOfLastVolume = @(_Volume + self.PreviousKlineModel.SumOfLastVolume.doubleValue);
        //        NSLog(@"%@======%@======%@------%@",_Close,self.MA7,self.MA30,_SumOfLastClose);
