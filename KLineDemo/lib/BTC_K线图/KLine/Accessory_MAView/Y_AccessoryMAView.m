...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _MACDLabel.textColor = [UIColor colorWithRGiOS:0xffffff];
...
-(void)maProfileWithModel:(Y_KLineModel *)model
{
    if(self.targetLineStatus != Y_StockChartTargetLineStatusKDJ)
    {
        ...
        _DIFLabel.text = [NSString stringWithFormat:@"  DIF：%.4f ",model.DIF.doubleValue];
        _DEALabel.text = [NSString stringWithFormat:@"  DEA：%.4f",model.DEA.doubleValue];
        _MACDLabel.text = [NSString stringWithFormat:@"  MACD：%.4f",model.MACD.doubleValue];
    } else {
